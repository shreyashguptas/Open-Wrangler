...
function createPreviewPanel(title, rows, rowCount, colCount) {
...
<body>
  ...
  <script>
    ...
    (0, logging_1.info)(`[webview] opened preview for title="${title}" rows=${rows.length} reportedRowCount=${rowCount ?? 'n/a'} reportedColCount=${colCount ?? 'n/a'}`);
    return panel;
}
